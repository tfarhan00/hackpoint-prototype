...
export default function NavbarCopy() {
  ...
  return (
    ...
    className='bg-white relative w-[100%] h-[50px] flex justify-between items-center px-4 lg:px-12'>
        <Link to='/'>
        ...
        <div className='items-center gap-4 hidden md:flex font-[500] text-xs'>
            ...
            <button onClick={(localStorage.getItem("username") == null) ? logIn : logOut} type="button" className="text-gray-900 focus:ring-4 focus:ring-gray-300 rounded-full px-4 py-1">{location.pathname !== '/login' ? 'Log Out' : 'Log In'}</button>
        </div> 
